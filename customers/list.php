...
$pageTitle = __('customer_database');
...
<!-- Modern Tailwind Customer List -->
<div class="space-y-6">
    <!-- Page Header -->
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div>
                <h1 class="text-2xl lg:text-3xl font-bold text-gray-900"><?php echo __('customer_database'); ?></h1>
                <p class="text-gray-600 mt-1"><?php echo __('manage_customers'); ?></p>
            </div>
            <div class="flex items-center space-x-3">
                ...
                <a href="../export_csv.php?type=customers" download class="inline-flex items-center px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-medium rounded-lg transition-colors duration-200">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div class="flex-1 max-w-md">
                <div class="relative">
                    ...
                    <input type="text" id="customerSearch" placeholder="<?php echo __('search_customers'); ?>..." class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-colors duration-200">
                </div>
            </div>
            <div class="flex items-center space-x-3">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <?php if (count($customers) > 0): ?>
                        <?php foreach ($customers as $customer): 
                            $status_class = $customer['balance'] > 0 ? 'status-warning' : 'status-success';
                            $status_text = $customer['balance'] > 0 ? 'Balance Due' : 'Current';
                            $status_color = $customer['balance'] > 0 ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800';
                        ?>
                            <tr class="hover:bg-gray-50 transition-colors duration-200" data-name="<?php echo strtolower($customer['name']); ?>" data-contact="<?php echo strtolower($customer['email'] . ' ' . $customer['phone']); ?>">
                                <td class="px-6 py-4">
                                    <div class="flex items-center space-x-3">
                                        <div class="w-10 h-10 bg-gradient-to-br from-purple-400 to-purple-600 rounded-lg flex items-center justify-center text-white font-bold text-sm">
                                            <?php echo substr($customer['name'], 0, 2); ?>
                                        </div>
                                        <div>
                                            <div class="text-sm font-semibold text-gray-900"><?php echo htmlspecialchars($customer['name']); ?></div>
                                            <div class="text-xs text-gray-500">Since <?php echo date('M Y', strtotime($customer['created_at'])); ?></div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    <div class="text-sm font-semibold text-gray-900"><?php echo number_format($customer['total_purchases'], 2) . ' ' . getSetting('currency_symbol', 'DH'); ?></div>
                                    <div class="text-xs text-gray-500">Total purchases</div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="text-sm font-semibold text-gray-900"><?php echo $customer['sale_count']; ?></div>
                                    <div class="text-xs text-gray-500">Orders</div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="text-sm font-semibold <?php echo $customer['balance'] > 0 ? 'text-red-600' : 'text-green-600'; ?>"><?php echo number_format($customer['balance'], 2) . ' ' . getSetting('currency_symbol', 'DH'); ?></div>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center space-x-2">
                                        <a href="view.php?id=<?php echo $customer['id']; ?>" class="inline-flex items-center px-3 py-1.5 bg-primary-100 hover:bg-primary-200 text-primary-700 text-xs font-medium rounded-lg transition-colors duration-200" data-tooltip="View Details">
                                            <i class="fas fa-eye mr-1"></i>
                                            View
                                        </a>
                                        <a href="edit.php?id=<?php echo $customer['id']; ?>" class="inline-flex items-center px-3 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs font-medium rounded-lg transition-colors duration-200" data-tooltip="Edit Customer">
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center">
                                ...
                                <p class="text-gray-600 mb-4">Start by adding your first customer to the system</p>
                                <a href="add.php" class="inline-flex items-center px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white text-sm font-medium rounded-lg transition-colors duration-200">
                                    <i class="fas fa-plus mr-2"></i>
