...
// Get user data
$stmt = $pdo->prepare("SELECT id, name, username, role, is_active FROM users WHERE id = ?");
    ...
    try {
        ...
        if (empty($errors)) {
            ...
            if (!empty($new_password)) {
                ...
                $stmt = $pdo->prepare("UPDATE users SET name = ?, username = ?, password_hash = ?, role = ?, is_active = ? WHERE id = ?");
                $stmt->execute([$name, $username, $password_hash, $role, $is_active, $user_id]);
            } else {
                $stmt = $pdo->prepare("UPDATE users SET name = ?, username = ?, role = ?, is_active = ? WHERE id = ?");
                $stmt->execute([$name, $username, $role, $is_active, $user_id]);
            }

            flash('success', __('user_updated_successfully'));
            header('Location: users.php');
            exit();
        }

    } catch (Exception $e) {
        $errors[] = __('error_updating_user') . $e->getMessage();
    }
}
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div>
                ...
                <p class="text-gray-600 mt-1"><?php echo __('edit_user_description'); ?></p>
            </div>
        </div>
    </div>
                    ...
                    <form method="POST" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="password" id="new_password" name="new_password"
                                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-colors duration-200">
                                <p class="mt-1 text-xs text-gray-500"><?php echo __('leave_blank_to_keep_current_password'); ?></p>
                            </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <select id="role" name="role" required
                                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-colors duration-200">
                                    <option value="cashier" <?php echo (($_POST['role'] ?? $user['role']) == 'cashier') ? 'selected' : ''; ?>><?php echo __('cashier_role'); ?></option>
                                    <option value="manager" <?php echo (($_POST['role'] ?? $user['role']) == 'manager') ? 'selected' : ''; ?>><?php echo __('manager_role'); ?></option>
                                    <option value="admin" <?php echo (($_POST['role'] ?? $user['role']) == 'admin') ? 'selected' : ''; ?>><?php echo __('admin_role'); ?></option>
                                </select>
                            </div>
                        ...
                        <div class="flex justify-end space-x-4 pt-4 border-t border-gray-200 mt-8">
                            ...
                            <button type="submit" class="px-6 py-2 bg-primary-600 hover:bg-primary-700 text-white text-sm font-medium rounded-lg transition-colors duration-200">
                                ...
                                <?php echo __('update_user_button'); ?>
                            </button>
                        </div>
                    </form>
                </div>
